/*
   ...
   VERSION HISTORY
   v2.2    Added proper namespace, added vformat, updated version - mthornton
...
class com.natecook.Sprintf
{
   ...
   /**
   Operates as format() but accepts an array of arguments, rather than a variable number of arguments.
   */
   static function vformat(format:String, args:Array):String
   {
	   args.unshift(format);
	   return Sprintf.format.apply(null, args);
   }
               ...
               while (fieldOutcomeSuccess != true && (formatIndex < format.length)) {
                   ...
                   switch (ch) {
                       ...
                       */
                       ...
                       case 'R':
                           fieldOutcomeSuccess = true;
                           destString += Sprintf.formatR(value,properties,length,precision,(ch == 'R'));
                           break;      
                       case '%':
                           fieldOutcomeSuccess = true;
                           destString += '%';
   ...
   static function formatF(value:Number,properties:Number,length:Number,precision:Number):String
   {
       ...
       if (Number(decPortion) == 0) {
           ...
           if (decPortion.length > precision) {
               ...
               var dec:Number = Math.round(pow10 * Number('0.' + decPortion));
               if (((String(dec)).length > precision) && (dec != 0)) {
               	   // In this case, we rounded up by one since the number was too close to one, like 0.999 truncate to %.2f becomes 1.0
                   decPortion = '0';
                   intPortion = ((Math.abs(Number(intPortion)) + 1) * (Number(intPortion) >= 0 ? 1 : -1)).toString();
               } else {
                   decPortion = new String(dec.toString());
                   
                   // If the string got shorter, leading zeroes must have been stripped
                   while(decPortion.length < precision) {
                       decPortion = '0'+decPortion;
                   }
               }
           }
   ...
   static function formatR(value:Object,properties:Number,length:Number,precision:Number,upper:Boolean):String
   {
	   var output:String = '';
	   if ( value.__repr__ != undefined )
	   {
		   output = value.__repr__();
	   }
       else if ( typeof value == 'string' || typeof value == 'number' || typeof value == 'boolean' )
       {
		   output = '"'+String(value)+'"';
       }
       else if( value.length != undefined and value[0] != undefined )
       {
		   output += '[';
		   for ( var i:Number = 0; i < value.length; i++ )
		   {
		   		if(i > 0) output += ', ';
				output += formatR(value[i]);
		   }
		   for ( var i in value )
		   {
		   		if(i < 0 || i >= value.length)
					output += ' ' + i + ' = ' + formatR(value[i]);
		   }
		   output += ']';
       }
       else if( typeof value == 'movieclip' )
       {
       		output = '<movieclip "'+value._name+'">';
       }
	   else
	   {
		   output += '<' + (typeof value);
