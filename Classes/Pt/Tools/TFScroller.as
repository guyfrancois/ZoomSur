...
class Pt.Tools.TFScroller {
	...
	private var onText:Boolean;
	/**
	 * 
	 * 	textfield:TextField;
	 * 	@param textfield :  le champs texte 
	 * 		le clip parent contiendra la barre de scroll
	 ...
	 * 			alignés en x=0;
	 */
	public function TFScroller(textfield:TextField,droite:Boolean,asScroller:String,margeX:Number,margeY:Number,onText:Boolean) {
		//trace("Pt.Tools.TextScoller.TextScoller("+textfield+", "+droite+", "+asScroller+")");
		clearScroller(textfield);
		if (margeX!=undefined) {
			this.margeX=margeX;
		} else {
			this.margeX=20;
	...
	private function _onWheel(src:Souris,incr:Number,scrollTarget:Object){
		//trace("Pt.Tools.TextScoller._onWheel(src, "+incr+")");
		if (scrollerMC==undefined) {
			src.stopScan();

		}
		if (onText && scrollTarget!=texte ) return;
		if (src.hasMouseOver() || Souris.hitTF(texte) ) {
	...
	static function clearScroller(tfScoll:TextField){
		var tfs:TFScroller= tfScoll.TFSCROLLER;
		tfs.removeScroller(); 
	}
	
	private function addScroller(){
		
		var scrollInstanceName:String="scrollerMC_"+texte._name;
		var scrollDepth:Number=texte._parent.getNextHighestDepth()
		if (texte._parent[scrollInstanceName]!=undefined) {
			scrollDepth=texte._parent[scrollInstanceName].getDepth();
			texte._parent[scrollInstanceName].removeMovieClip();
		}
		
		if (droite) {
			scrollerMC=texte._parent.attachMovie(asScroller,scrollInstanceName,scrollDepth,{_x:_baseXText,_y:Math.floor(_baseYText+margeY)});
		...
		scrollerMC.onUnload=Delegate.create(this, destroy);
	...
	private function  initScrollerMover(){
        ...
        scrollerMC.btn_haut.addEventListener("onRollOver",onOverAuto,this);
        scrollerMC.btn_haut.addEventListener("onRollOut",onOutAuto,this);
       // scrollerMC.btn_haut.addEventListener("onPress",onHaut,this);
       // scrollerMC.btn_haut.addEventListener("onRelease",onOver,this);
        scrollerMC.btn_haut.addEventListener("onReleaseOutside",onOutAuto,this);
        
        scrollerMC.btn_bas.addEventListener("onRollOver",onOverAuto,this);
        scrollerMC.btn_bas.addEventListener("onRollOut",onOutAuto,this);
      //  scrollerMC.btn_bas.addEventListener("onPress",onBas,this);
      //  scrollerMC.btn_bas.addEventListener("onRelease",onOver,this);
        scrollerMC.btn_bas.addEventListener("onReleaseOutside",onOutAuto,this);
	...
	function startScan() {
		...
		manetteYMouse=scrollerMC.manette._ymouse*scrollerMC.manette._yscale/100;
...
//          locRef.update();
	...
	function _onMouseMove(){
		var nextY=scrollerMC._ymouse-manetteYMouse;
	...
	private function updateScoller(){
		...
		//trace("Pt.Tools.TFScroller.updateScoller()"+texte.maxscroll);
		var nbligneVisible:Number=texte.bottomScroll-texte.scroll;
		//trace("Pt.Tools.TFScroller.updateScoller()"+nbligneVisible+" "+texte.maxscroll);
		scrollerMC.manette._height=scrollerMC.fond._height*nbligneVisible/(nbligneVisible+texte.maxscroll);
		//trace("Pt.Tools.TFScroller.updateScoller()"+scrollerMC.manette._yscale);
	...
	/**
	 * Mode Auto sur btn	 */
	private function onOverAuto(clip:MovieClip){
		switch (clip) {
			case scrollerMC.btn_bas :
				temopScroll=new Temporise(50,Delegate.create(this, btnOver,-1))
			break;
			case scrollerMC.btn_haut :
				temopScroll=new Temporise(50,Delegate.create(this, btnOver,1))
			break;
		}
	...
	private function onOutAuto(clip:MovieClip) {
		clip.gotoAndStop(IMG_OUT);
		temopScroll.destroy();
	}
	
	function btnOver(tmp:Temporise,count:Number,sens:Number){
		scrollTo(getYPos()+scrollerMC.fond._y-sens*pas);
	}
}
