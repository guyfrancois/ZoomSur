...
class Pt.Tools.ClipScoller extends EventDispatcher{
	/**
	...
	public function ClipScoller(clipTexte:MovieClip,droite:Boolean,asScroller:String) {
		super();
		...
		souris.addEventListener(Souris.ON_MOUSEWHEEL,this._onWheel,this);
	}
	/**
	...
	private function addScroller(){
		souris.startScan();
		if (droite) {
			scrollerMC=clipTexte.attachMovie(asScroller,"scrollerMC",10,{_x:_baseXText,_y:_baseYText});
			
			//texte._width=_maxWidthText-scrollerMC._width-2;
			scrollerMC._x=_baseXText+_maxWidthText+marge;
		
		} else {
			scrollerMC=clipTexte.attachMovie(asScroller,"scrollerMC",10,{_x:_baseXText,_y:_baseYText});
			//texte._x=_baseXText+scrollerMC._width+marge;
			scrollerMC._x=_baseXText-(scrollerMC._width+marge);
    ...
    private function scrollTo(nextY:Number) {
        ...
        updateTextScoller(scrollerMC.fond._height-scrollerMC.manette._height,scrollerMC.manette._y);
    }
	...
	private function updateScoller(){
		scrollerMC.fond._height=_maxHeightText;
		scrollerMC.manette._y=0;
		scrollerMC.btn_bas._y=Math.floor(scrollerMC.fond._y+scrollerMC.fond._height+7);
