...
class Pt.Tools.ClipScoller extends EventDispatcher implements Pt.conteneur.I_resizable {
	static var TWEENSPEED:Number=1;
	/**
	...
	*/
	...
	public function ClipScoller(clipTexte:MovieClip,droite:Boolean,asScroller:String,margeX:Number,margeY:Number) {
		super();
	...
	/**
	 * le texte à changé	 */
	public function onChanged(enPlace:Boolean){
		//trace("Pt.Tools.ClipScoller.onChanged("+enPlace+")"+texte._x);
		...
		texte._x=_baseXText;
		texte._y=_baseYText;
		if (texte._height>_maxHeightText) {
	...
	private function restorTextPos(){
		//trace("Pt.Tools.ClipScoller.restorTextPos()"+texte._x);
		souris.stopScan();
		texte._x=_baseXText;
		texte._y=_baseYText;
	...
	private function removeScroller(){
		//trace("Pt.Tools.ClipScoller.removeScroller()"+texte._x);
		souris.stopScan();
	...
	private function onPressFond(){
	  ...
	  function get pas():Number {
	  	return Math.floor((scrollerMC.fond._height-scrollerMC.manette._height)/(texte._height/30));
       // return 5;
    }
   ...
   public function getYPos():Number {
    ...
    public function set y(val:Number) {
    	//trace("Pt.Tools.ClipScoller.y("+val+")");
    	moveTo(val);
    }
    
    public function tween(y:Number) {
    	//trace("Pt.Tools.ClipScoller.tween("+y+")"+texte._y);
    	if (y==undefined ){ y=0;
    	} 
    	Tweener.addTween(this,{ y:y ,time:TWEENSPEED })
    }
...
//          locRef.update();
	...
	private function updateScoller(){
		...
		*/
		...
		scrollerMC.fond._height=Math.floor(_maxHeightText-btnSpace-margeY*2);
		...
		scrollerMC.manette._height=scrollerMC.fond._height/(texte._height/_maxHeightText);    
		//("scrollerMC.fond._height:"+scrollerMC.fond._height)	;
		//trace("_maxHeightText:"+_maxHeightText)	 ;
		//trace("texte._heigh:"+texte._height)                                 ;//scrollerMC.fond._height*nbligneVisible/(nbligneVisible+texte.maxscroll);
	...
	private function updateTextScoller(height:Number,ypos:Number){
		...
		texte._y=Math.floor(_baseYText-Math.floor(ypos*(texte._height-_maxHeightText)/height));
	...
	private function _moveTo(tagypos:Number){
		...
		ypos=(tagypos)*height/(texte._height-_maxHeightText);
		temporiseScroll.destroy();
		/*
		...
		*/
		scrollTo(ypos);
		//trace("Pt.Tools.ClipScoller.scrollTo("+tagypos+")=="+(-texte._y));
	...
	public function toHidePlace(){
		//trace("Pt.Tools.ClipScoller.toHidePlace()"+texte._x);
