 ...
 */
class GraphicTools.MenuPlat extends EventDispatcher {
	/**
	 * Evenement
	...
	public function MenuPlat(clip:MovieClip,resetEnable:Number,imagesBtn:Object,dataXML:Object,withInitialise:Boolean, reclic:Boolean) {
		super();
		this.reclic=(reclic==undefined)?false:reclic;
		//trace("GraphicTools.MenuPlat.MenuPlat("+clip+", "+resetEnable+", "+imagesBtn+" , dataXML, "+withInitialise+" reclic"+reclic+")");
	...
	public function initialize (defSize:Number){
		...
		//trace("GraphicTools.MenuPlat.initialize(defSize):"+size);
		if (size==undefined) {
			size=defSize;
		} 
		_size=size;
	...
	private function createBtn(i:Number):GraphicTools.BOverOutSelect {
		...
		//trace("GraphicTools.MenuPlat.createBtn("+i+")"+clip);
	 ...
	 * appelé lorsque le contenu d'un bouton est initialisé	 */
	 private function disposeBtn(i:Number){
	 	//trace("GraphicTools.MenuPlat.disposeBtn("+i+")");
	 	var lClip:MovieClip=clip[_btnPrefix+i];
