...
/**
  ...
  *   */
class GraphicTools.MenuVOver extends EventDispatcher {
	...
	public function MenuVOver(clip:MovieClip,arboMenu:Object,margeHaute:Number,autoSubOpen:Boolean) {
		super();
		selected=false;
	...
	private function initClipControle(){
		...
		souris=new OverOut(clip);
		souris.startScan(true);
		souris.addEventListener(OverOut.ON_ROLLOVER,deroule,this);
		souris.addEventListener(OverOut.ON_ROLLOUT,closeMenu,this);
 ...
 * internal, reservé à la fermeture du clip (enterFrame) */
	...
	private function createChild_Menu(child:MovieClip,decalageY:Number,autoSubOpen:Boolean,struct:Object){
		return new GraphicTools.MenuV(child,struct,child._y-decalageY,autoSubOpen);
	}
	/**
	...
	private function updateSize(){
		...
		for (var i:Number =1; i<menuObjectArray.length;i++) {
			...
			if ( prev instanceof GraphicTools.MenuV && prev.isOpen()) {  
	...
	private function _onClosed(src:MenuVOver){
